...
const EnrolledClasses = () => {
  ...
  const { data: enrolledClasses = [], refetch } = useQuery({
    ...
    enabled: !loading,
    queryFn: async () => {
      const res = await axiosSecure.get(`/classes/enroll/${user?.email}`);
      return res.data;
    },
  });
  ...
  const handleDeleteClass = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/enrolledClasses/${id}`).then((res) => {
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {enrolledClasses.map((enrolledClass, index) => (
                            <tr key={index}>
                              ...
                              <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5">
                                <p>{enrolledClass?.instructorName}</p>
                              </td>
                              <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5">
                                <p>${enrolledClass?.price}</p>
                              </td>
